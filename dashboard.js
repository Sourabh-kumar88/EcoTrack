...
async function checkAuthentication() {
    try {
        ...
        if (error || !session) {
            ...
            console.warn('No valid session found, redirecting to signin');
...
db.auth.onAuthStateChange((event, session) => {
    if (!session && authChecked) {
        ...
        console.warn('Session ended, redirecting to signin');
        window.location.href = '/signin.html';
    } else if (session) {
        currentSession = session;
    }
...
function setupDashboardDOM() {
    ...
    if (!scoreRing) console.warn('scoreRing element not found');
    if (!scoreValue) console.warn('scoreValue element not found');
    if (!journalForm) console.warn('journalForm element not found');
    if (!journalFeed) console.warn('journalFeed element not found');
    ...
    function checkDailyReset() {
        ...
        if (lastResetDate !== today) {
            ...
            console.log('✓ Daily reset triggered for', today);
            return true;
        }
        return false;
    }
     ...
     */
    function addJournalEntryDOM(text, meta, logId = null) {
        ...
        entry.innerHTML = `
            ...
            ${logId ? `<button type="button" class="journal-delete-btn" data-log-id="${logId}" title="Delete log" aria-label="Delete log">
    ...
    /**
     * Delete a log entry from storage and refresh the dashboard
     */
            ...
            if (!data || data.length === 0) {
                ...
                data.forEach(log => {
                    ...
                    if (log.details.startsWith('"')) {
                         addJournalEntryDOM(log.details, `Explorer • ${time}`, log.id);
                    } else {
                         addJournalEntryDOM(`Logged ${log.amount.toFixed(1)} kg CO2e for ${log.details}`, `${log.category} • ${time}`, log.id);
                    }
                });
            }
    ...
    // Delegate delete clicks on journal entries
    if (journalFeed) {
        journalFeed.addEventListener('click', async (e) => {
